...
int ModemClass::begin(bool restart)
{
  ...
  if (!isPowerOn()) {
    ...
    delay(1000); // Datasheet says power-on pulse should be >=150ms, <=3200ms
...
void ModemClass::end()
{
  ...
  if (isPowerOn()) {
    digitalWrite(_powerOnPin, HIGH);
    delay(2000); // Datasheet says power-off pulse should be >=1500ms
    digitalWrite(_powerOnPin, LOW);
    setVIntPin(SARA_VINT_OFF);
  }
}
...
{
  for (unsigned long start = millis(); (millis() - start) < timeout;) { //timeout is fixed
    if (noop() == 1) {
      return 1;
    }
      ...
      default: {
        ...
        if (_buffer.startsWith("AT") && _buffer.endsWith("\r\n")) {
          ...
          if (_buffer.length()) {
            ...
            for (int i = 0; i < MAX_URC_HANDLERS; i++) { //fixed
